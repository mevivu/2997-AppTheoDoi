<div class="position-relative d-inline-block">
    ...
    @if($video_url)
        @if(($video_type ?? 'youtube') === 'r2')
            <a href="{{ $video_url }}" target="_blank" class="position-absolute top-50 start-50 translate-middle" title="{{ __('Xem video Cloudflare R2') }}">
                <span class="avatar avatar-xs bg-orange text-white rounded-circle shadow" style="width: 24px; height: 24px;">
                    <i class="ti ti-cloud fs-14"></i>
                </span>
            </a>
        @else
