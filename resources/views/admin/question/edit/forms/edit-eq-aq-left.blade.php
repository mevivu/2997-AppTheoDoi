        ...
        <div class="row card-body">
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    @if ($response->question_type->value == QuestionType::EQ->value)
                        <input type="text" value="EQ" class="form-control" disabled>
                        <input type="hidden" name="question[question_type]" value="eq">
                    @elseif($response->question_type->value == QuestionType::AQ->value)
                        ...
                        <x-select name="question[question_group_id]" :required="true">
                            @foreach ($questionGroups as $key => $value)
                                <x-select-option :value="$key" :title="$value"
                                                 :selected="$response->question_group_id == $key"/>
                            @endforeach
                        </x-select>
                    </div>
                </div>
                    ...
                    <div class="card-body p-2">
                        ...
                        <div class="preview-image">
                            <img id="preview-image"
                                 src="{{ $response->question_image ? asset($response->question_image) : asset('/public/assets/images/default-image.png') }}"
                                 style="max-width: 100%;">
                        </div>
                        @if($response->question_image)
                            <button type="button" id="remove-image" class="btn btn-danger" onclick="removeImage()">
                                Xoá
                                ảnh
                            </button>
                ...
                <div class="mb-3">
                    ...
                    <x-select name="question[age_group]" :required="true">
                        @foreach ($age_group as $key => $value)
                            <x-select-option :value="$key" :title="$value"
                                             :selected="$response->age_group && $response->age_group->value == $key"/>
                        @endforeach
                    </x-select>
                </div>
            </div>
            ...
            <div class="col-12">
                <div class="d-flex align-items-center justify-content-between">
                    <label
                        class="control-label">{{ __('Câu trả lời (Nhập điểm đánh giá ở mục đầu tiên của mỗi câu trả lời)') }}
                        :</label>
