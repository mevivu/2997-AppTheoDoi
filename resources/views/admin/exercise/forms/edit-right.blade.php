...
<div class="col-12 col-lg-4 col-xl-3">
        ...
        <div class="card-body p-4">
            <x-select name="access_type">
                @foreach ($accessTypes as $key => $value)
                    <x-select-option :value="$key" :title="$value" :selected="($response->access_type?->value ?? '') == $key" />
                @endforeach
            </x-select>
    ...
    <div class="card border-0 custom-shadow rounded-3 mb-4">
        ...
        <div class="card-body p-4">
            <x-input type="number" name="sort_order" :value="old('sort_order', $response->sort_order ?? 0)" min="0" />
        </div>
    </div>
