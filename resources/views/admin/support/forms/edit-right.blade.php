        ...
        <div class="card-body p-4">
            <x-select name="status" :required="true">
                @foreach ($status as $key => $value)
                    <x-select-option :value="$key" :title="$value" :selected="old('status', $response->status->value) == $key" />
                @endforeach
            </x-select>
        </div>
    </div>
        ...
        <div class="card-body p-4">
            <x-select name="type" :required="true">
                @foreach ($type as $key => $value)
                    <x-select-option :value="$key" :title="$value" :selected="old('type', $response->type->value) == $key" />
                @endforeach
            </x-select>
        </div>
    </div>
